...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    fs::create_dir_all(temp_dir.join(".hackcode")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".hackcode.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".hackcode").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    assert!(stdout.contains(temp_dir.join(".hackcode.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".hackcode")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("hackcode doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            hackcode doctor"));
    ...
    assert!(status_stdout.contains("Usage            hackcode status"));
