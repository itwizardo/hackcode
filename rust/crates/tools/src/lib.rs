...
pub fn hackcode_tool_specs() -> Vec<ToolSpec> {
    mvp_tool_specs()
        .into_iter()
        .filter(|spec| HACKCODE_TOOLS.contains(&spec.name))
        .collect()
}
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("hackcode-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("HACKCODE_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    ...
    Ok(cwd.join(".hackcode-todos.json"))
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(claw_config_home) = std::env::var("HACKCODE_CONFIG_HOME") {
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.hackcode/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".hackcode"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".hackcode"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("hackcode-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".hackcode-agents"));
    }
    Ok(cwd.join(".hackcode-agents"))
}
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".hackcode").join("settings.local.json"),
    })
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("hackcode-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .hackcode/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let claw_dir = worktree.join(".hackcode");
        fs::create_dir_all(&claw_dir).expect("create .hackcode dir");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .hackcode/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".hackcode").join("worker-state.json");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".hackcode").join("skills").join("plan");
        let command_dir = root.join(".hackcode").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".hackcode/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".hackcode/commands/handoff.md"));
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".hackcode")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".hackcode")).expect("cwd dir");
        std::fs::write(
            home.join(".hackcode").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".hackcode")).expect("cwd dir");
        std::fs::write(
            cwd.join(".hackcode").join("settings.local.json"),
